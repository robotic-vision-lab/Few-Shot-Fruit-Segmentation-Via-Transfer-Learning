...
class TriBranch(nn.Module):
    def __init__(self, n_classes, aux_mode='train', in_chans=3, embed_dims=[64, 128, 256, 512],
        ...
        #Derivative Branch (code from PIDNet https://github.com/XuJiacong/PIDNet)
        ...
        self.layer5_d = self._make_layer(Bottleneck, embed_dims[1], embed_dims[0] * 2, 1)
        ...
        if self.aux_mode == 'train':
            self.conv_out_ = SegmentHead(embed_dims[3], embed_dims[2], n_classes, up_factor=32)
            self.conv_out8 = SegmentHead(embed_dims[1], embed_dims[1], n_classes, up_factor=8)
    ...
    def forward(self, x):
        ...
        feat_out = self.conv_out(self.dfm(feat_, feat, x_d))
        if self.aux_mode == 'train':
            feat_out8 = self.conv_out8(feat8)
            feat_out_ = self.conv_out_(feat32)
            feat_d = self.seghead_d(temp_d)
            return feat_out, feat_out8, feat_out_, feat_d
